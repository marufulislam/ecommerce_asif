    ...
    <div class="container">
      <div class="row">
        ...
        <div id="content" class="col-sm-9">
          ...
          <div class="owl-carousel product_carousel" id="shop">
          	
            <?php
            $sql_art = "SELECT cat.category_name,prod.* FROM category as cat,products as prod WHERE cat.category_id=prod.category_id  ORDER BY prod.product_id DESC limit 0,4";
            $qry_art = mysql_query($sql_art) or die ('Error in home slider selection->'.mysql_error());
            if(mysql_num_rows($qry_art) > 0){
				
				while($row_art=mysql_fetch_array($qry_art)){
				$product_id=$row_art['product_id'];
				
				$sq = mysql_query("SELECT image_name FROM product_image WHERE main_image = '0' OR main_image = '1' AND product_id='".$row_art['product_id']."'");
				while($row_slider=mysql_fetch_array($sq)){
					$img='../products/'.$row_slider['image_name'];
					?>
            <div class="product-thumb clearfix item" idLatest="<?php echo $product_id;?> id="product_<?php echo $row_art['product_id'];?>
        			data-id="<?php echo $row_art['product_id'];?>" 
        			data-photo="../products/<?php echo $row_art['image_name'];?>" 
        			data-name="<?php echo $row_art['product_name'];?>" 
        			data-price="<?php echo $row_art['price_in_usd'];?>">
              ...
              <div class="caption">
                <h4><a href="product.html"><?php echo $row_art['product_name'];?></a></h4>
                 <?php
                        $amount=$row_art['price_in_usd'];
                        $discount=$row_art['product_discount_percentage'];
                        $total = $amount*(1-$discount/100);
                 ?>
                  <?php if($row_art['product_discount_percentage'] > 0){ ?>
                       <p class="price"><span class="price-new priceAmountFormat"><?php echo $total;?></span><span class="price-old priceAmountFormat"> <?php echo $row_art['price_in_usd'];?></p>
                        <?php } else{?>
                         <p class="price"><span class="price-new priceAmountFormat"><?php echo $row_art['price_in_usd'];?></span></p>

                        <?php } ?>
               
                
                 <?php if($row_art['product_discount_percentage'] > 0){ ?>
                <p><span class="saving priceAmountFormat">- <?php echo $row_art['product_discount_percentage'];?>%</span></p>
                 <?php } ?>
              </div>
              <div class="button-group">
               <a class="btn-primary add-to-cart">
          ...
          <div class="category-module" id="latest_category">
            <h3 class="subtitle">Electronics - <a class="viewall" href="category-2.html">view all</a></h3>
